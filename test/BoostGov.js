...
contract('BoostGov', ([governance, alice, bob, carol]) => {
  before('init uniswap, tokens, and governance', async () => {
    ...
    await this.boost.transfer(carol, web3.utils.toWei('10000'), {from: governance});
  ...
  it('should revert when proposing due to not staking BOOST', async () => {
    await expectRevert(
      this.gov.propose('https://gov.boosted.finance/', web3.utils.toWei('1'), alice, {from: alice}),
      'stake more boost'
    );
  });
  ...
  it('should successfully vote for proposal', async () => {
    ...
    const proposal = await this.gov.proposals(0);
    assert.equal(proposal['totalForVotes'], web3.utils.toWei('100'));
  });
